...
parser.add_argument('--save_dir', type=str, default='experiment_output/', 
...
parser.add_argument('--icvf_type', type=str, default='multilinear', 
                    choices=list(icvfs.keys()), help='Which model to use.')
...
def main():
    ...
    writer = SummaryWriter(log_dir=os.path.join('runs', args.experiment_id))
    ...
    # Create ICVF model
    value_def = create_icvf(args.icvf_type, hidden_dims=hidden_dims)
    ...
    for step in tqdm(range(1, args.max_steps + 1)):
        ...
        if step % args.log_interval == 0:
            ...
            for k, v in update_info.items():
                writer.add_scalar(f'training/{k}', v, step)
    ...
    def generate_debug_plots(self, agent):
        ...
        if 'maze' in self.env_name:
            ...
            for idx in range(9):
                ...
                image = viz_library.plot_policy(viz_env, viz_dataset, policy_fn)
                policy_images.append(image)
            visualizations['intent_policies'] = torch.stack(policy_images)
            
            # Value visualization
            value_images = []
            for idx in range(9):
                value_fn = functools.partial(get_values, agent, intent=intents[idx])
                image = viz_library.plot_value(viz_env, viz_dataset, value_fn)
                value_images.append(image)
            visualizations['intent_values'] = torch.stack(value_images)
...
def get_debug_statistics(agent, batch):
    ...
    info_ssz = agent.value.get_info(s, s, z)
    info_szz = agent.value.get_info(s, z, z)
    info_sgz = agent.value.get_info(s, g, z)
    info_sgg = agent.value.get_info(s, g, g)
    info_szg = agent.value.get_info(s, z, g)
...
def get_v_zz(agent, goal, observations):
    ...
    goal_tiled = goal.repeat(observations.size(0), 1)
...
def get_v_gz(agent, initial_state, target_goal, observations):
    ...
    initial_state_tiled = initial_state.repeat(observations.size(0), 1)
    target_goal_tiled = target_goal.repeat(observations.size(0), 1)
...
def get_traj_v(agent, trajectory):
    ...
    for i in range(n):
        for j in range(n):
            ...
            all_values[i, j] = agent.value(s, g, g).mean().item()
